...
function cacheElements() {
  ...
  UI.installBanner = document.getElementById('installBanner');
  UI.btnInstallApp = document.getElementById('btnInstallApp');
  UI.btnDismissInstall = document.getElementById('btnDismissInstall');
  UI.infoModal = document.getElementById('infoModal');
  UI.infoModalEyebrow = document.getElementById('infoModalEyebrow');
  UI.infoModalTitle = document.getElementById('infoModalTitle');
  UI.infoModalBody = document.getElementById('infoModalBody');
  UI.btnOpenAbout = document.getElementById('btnOpenAbout');
  UI.btnOpenPrivacy = document.getElementById('btnOpenPrivacy');
  UI.btnCloseInfoModal = document.getElementById('btnCloseInfoModal');
...
function bindUIEvents() {
  ...
  UI.btnOpenAbout.addEventListener('click', openAboutModal);
  UI.btnOpenPrivacy.addEventListener('click', openPrivacyModal);
  UI.btnCloseInfoModal.addEventListener('click', closeInfoModal);
  UI.infoModal.addEventListener('click', (event) => { if (event.target === UI.infoModal) closeInfoModal(); });
  UI.btnInstallApp.addEventListener('click', installPWA);
  UI.btnDismissInstall.addEventListener('click', dismissInstallBanner);
...
function setupInstallPrompt() {
  window.addEventListener('beforeinstallprompt', (event) => {
    ...
    if (localStorage.getItem('radarmaparj-install-dismissed') !== 'yes') UI.installBanner.hidden = false;
  });

  window.addEventListener('appinstalled', () => {
    AppState.deferredInstallPrompt = null;
    UI.installBanner.hidden = true;
    showToast('RadarMapa RJ instalado com sucesso.', 'success');
  });
}
...
async function installPWA() {
  if (!AppState.deferredInstallPrompt) {
    UI.installBanner.hidden = true;
    showToast('Use o menu do navegador para adicionar o app à tela inicial.', 'info');
    return;
  }
  AppState.deferredInstallPrompt.prompt();
  try { await AppState.deferredInstallPrompt.userChoice; } catch (error) { console.warn(error); }
  AppState.deferredInstallPrompt = null;
  UI.installBanner.hidden = true;
}
...
function dismissInstallBanner() {
  UI.installBanner.hidden = true;
  localStorage.setItem('radarmaparj-install-dismissed', 'yes');
}

function openAboutModal() {
  UI.infoModalEyebrow.textContent = 'RadarMapa RJ';
  UI.infoModalTitle.textContent = 'Sobre, fontes e uso responsável';
  UI.infoModalBody.innerHTML = `
    <h3>Finalidade</h3><p>O RadarMapa RJ é um aplicativo web informativo e educativo para visualização de radares fixos e portáteis em rodovias do Estado do Rio de Janeiro.</p>
    <h3>Fontes de dados</h3><ul><li>Dados públicos do DER-RJ tratados em planilha própria.</li><li>Mapa base do OpenStreetMap.</li><li>Busca de endereço por Nominatim/OpenStreetMap.</li><li>Cálculo de rota por OSRM.</li></ul>
    <h3>Uso responsável</h3><div class="notice-box"><p>As informações exibidas têm caráter informativo. A sinalização viária local, os limites oficiais da via e as orientações das autoridades sempre prevalecem.</p><p>O objetivo do projeto é apoiar segurança viária, planejamento de rota e transparência sobre dados públicos.</p></div>
    <h3>Funcionamento offline</h3><p>A estrutura do app pode ser aberta mais rapidamente após a instalação, mas mapa, busca, GPS, rotas e planilha dependem de conexão com a internet para funcionar plenamente.</p>`;
  openInfoModal();
}

function openPrivacyModal() {
  UI.infoModalEyebrow.textContent = 'RadarMapa RJ';
  UI.infoModalTitle.textContent = 'Privacidade';
  UI.infoModalBody.innerHTML = `
    <h3>Localização</h3><p>O app solicita a localização apenas quando o usuário usa recursos como “Usar minha localização” ou “Iniciar rota”. A posição é processada no navegador para centralizar o mapa e emitir alertas de proximidade.</p>
    <h3>Serviços externos</h3><p>O app pode consultar OpenStreetMap/Nominatim, OSRM e Google Planilhas. Esses serviços podem receber dados técnicos da requisição, como endereço IP e termos pesquisados.</p>
    <h3>Permissões</h3><ul><li><strong>GPS:</strong> localização e acompanhamento da rota.</li><li><strong>Notificações:</strong> alerta opcional de radar próximo.</li><li><strong>Vibração e áudio:</strong> reforço de alertas.</li><li><strong>Wake Lock:</strong> tentativa de manter a tela ativa durante a navegação.</li></ul>
    <h3>Armazenamento local</h3><p>O app pode salvar preferências simples no navegador, como ocultar o banner de instalação.</p>`;
  openInfoModal();
}

function openInfoModal() { UI.infoModal.hidden = false; UI.btnCloseInfoModal.focus(); }
function closeInfoModal() { if (UI.infoModal) UI.infoModal.hidden = true; }
